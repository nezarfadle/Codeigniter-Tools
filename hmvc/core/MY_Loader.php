...
class MY_Loader extends CI_Loader{
	...
	{
		// temporary save the model's name
		$tempModelName = $model ;
		...
		$tempModelName = explode('/', $tempModelName);

		foreach($hmvc_paths as $key => $value){
			$hmvc_paths[$key] =  APPPATH . $value . $class . '/';

			if(is_array($tempModelName)){
				$hmvc_paths[] =  APPPATH . $value ;//.  $tempModelName[0] . '/';	
			}
	...
	public function view($view, $vars = array(), $return = FALSE)
	{
		...
		// get the the controller's name if nested 
		...
		foreach($hmvc_paths as $path){
			...
			// check out whether the view is in another controlelr outside the caller
			// controller
